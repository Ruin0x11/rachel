...
function repl:init(app, frame)
	...
	local console = styled_text_ctrl.create(
		frame,
		wx.wxID_ANY,
...
function repl:caretOnPromptLine(disallowLeftmost, line)
	...
	return (currentLine > promptLine or currentLine == promptLine and self:positionInLine(promptLine) > boundary)
end
...
function repl:getInput(line)
	...
	return chomp(
		self.console:GetTextRangeDyn(self.console:PositionFromLine(line), self.console:PositionFromLine(nextMarker))
	)
end
...
function repl:ConsoleSelectCommand(point)
	...
	if promptline == wxstc.wxSTC_INVALID_POSITION then
		return
	end
	local nextline = self.console:MarkerNext(promptline + 1, util.ANYMARKERMASK)
	local epos = nextline ~= wxstc.wxSTC_INVALID_POSITION and self.console:PositionFromLine(nextline)
		or self.console:GetLength()
...
function repl:getNextHistoryLine(forward, promptText)
	...
	if currentHistory == self:getPromptLine() or self:getInput(currentHistory) == promptText then
...
function repl:getNextHistoryMatch(promptText)
	...
	while true do
		...
		elseif currentHistory ~= self:getPromptLine() then -- skip current prompt
			...
			if input:find(promptText, 1, true) == 1 then
				return input
			end
		end
		-- couldn't find anything and made a loop; get out
		if currentHistory == current then
			return
		end
	end
...
function repl:load_history()
	...
	for line in f:lines() do
		...
		local text = self.console.useraw and line
			or util.fix_utf8(line, function(s)
...
function repl:shellPrint(marker, text, newline)
	if not text or text == "" then
		return
	end -- return if nothing to print
	if newline then
		text = text:gsub("\n+$", "") .. "\n"
	end
	...
	local insertAt = isPrompt and not partial and self.console:PositionFromLine(self:getPromptLine())
		or self.console:GetLength()
	self.console:InsertTextDyn(insertAt, self.console.useraw and text or util.fix_utf8(text, function(s)
		return "\\" .. string.byte(s)
	end))
	local linesAdded = self.console:GetLineCount() - lines
...
repl.displayShellDirect = function(self, ...)
	self:shellPrint(nil, concat("\t", ...), true)
end
repl.DisplayShell = function(self, ...)
	self:shellPrint(OUTPUT_MARKER, concat("\t", ...), true)
end
repl.DisplayShellErr = function(self, ...)
	self:shellPrint(ERROR_MARKER, concat("\t", ...), true)
end
repl.DisplayShellMsg = function(self, ...)
	self:shellPrint(MESSAGE_MARKER, concat("\t", ...), true)
end
-- don't print anything; just mark the line with a prompt mark
repl.DisplayShellPrompt = function(self, ...)
	self.console:MarkerAdd(self.console:GetLineCount() - 1, PROMPT_MARKER)
end
...
function repl.filterTraceError(err, addedret)
	...
	if addedret then
		err = err:gsub('^%[string "return ', '[string "')
	end
	err = err:match("(.*)\n[^\n]*%(tail call%): %?$") or err
	return err
end
...
local function packResults(status, ...)
	return status, { ... }
end
...
function repl:executeShellCode(tx)
	...
	elseif fn then
		...
		local ok, res = packResults(xpcall(fn, function(err)
...
function repl:bind_console_events()
	self.console.Print = function(_, ...)
		return self:DisplayShell(...)
	end
	self.console.Write = function(_, ...)
		return self:shellPrint(OUTPUT_MARKER, concat("", ...), false)
	end
	self.console.Error = function(_, ...)
		return self:DisplayShellErr(...)
	end

	self.console:Connect(wx.wxEVT_KEY_DOWN, function(event)
		...
		while true do
			...
			if key == wx.WXK_UP or key == wx.WXK_NUMPAD_UP then
				...
				if self.console:GetCurrentLine() > self:getPromptLine() then
					break
				end

				-- if we are not on the caret line, or are on wrapped caret line, move normally
				if not self:caretOnPromptLine() or self.console:GetLineWrapped(self.console:GetCurrentPos(), -1) then
					break
				end
			...
			elseif key == wx.WXK_DOWN or key == wx.WXK_NUMPAD_DOWN then
				...
				if self.console:GetCurrentLine() < totalLines then
					break
				end
			...
			elseif key == wx.WXK_TAB then
				...
				local promptline = self:getPromptLine()
				if self.console:GetCurrentLine() < promptline then
					return
				end
			...
			elseif key == wx.WXK_BACK or key == wx.WXK_LEFT or key == wx.WXK_NUMPAD_LEFT then
				if
					(
						key == wx.WXK_BACK
						or self.console:LineFromPosition(self.console:GetCurrentPos()) >= self:getPromptLine()
					) and not self:caretOnPromptLine(true)
				then
					return
				end
			elseif key == wx.WXK_DELETE or key == wx.WXK_NUMPAD_DELETE then
				if
					not self:caretOnPromptLine()
					or self.console:LineFromPosition(self.console:GetSelectionStart()) < self:getPromptLine()
				then
					return
				end
			elseif
				key == wx.WXK_PAGEUP
				or key == wx.WXK_NUMPAD_PAGEUP
				or key == wx.WXK_PAGEDOWN
				or key == wx.WXK_NUMPAD_PAGEDOWN
				or key == wx.WXK_END
				or key == wx.WXK_NUMPAD_END
				or key == wx.WXK_HOME
				or key == wx.WXK_NUMPAD_HOME
				-- `key == wx.WXK_LEFT or key == wx.WXK_NUMPAD_LEFT` are handled separately
				or key == wx.WXK_RIGHT
				or key == wx.WXK_NUMPAD_RIGHT
				or key == wx.WXK_SHIFT
				or key == wx.WXK_CONTROL
				or key == wx.WXK_ALT
			then
				break
			elseif key == wx.WXK_RETURN or key == wx.WXK_NUMPAD_ENTER then
				...
				if self:caretOnPromptLine(true) and event:ShiftDown() then
					break
				end
	...
	if util.version_geq(util.wx_version(), "2.9.5") then
		...
		local promptAt
		self.console:Connect(wxstc.wxEVT_STC_MODIFIED, function(event)
			local evtype = event:GetModificationType()
			if bit.band(evtype, wxstc.wxSTC_MOD_BEFOREINSERT) ~= 0 then
				local promptLine = self:getPromptLine()
				if promptLine and event:GetPosition() == self.console:PositionFromLine(promptLine) then
					promptAt = promptLine
				end
			end
		...
		self.console:Connect(wxstc.wxEVT_STC_UPDATEUI, function(event)
			self.console:SetReadOnly(not self:inputEditable())
		end)

		-- only allow copy/move text by dropping to the input line
		self.console:Connect(wxstc.wxEVT_STC_DO_DROP, function(event)
		...
		if config.nomousezoom then
			...
			self.console:Connect(wx.wxEVT_MOUSEWHEEL, function(event)
				if wx.wxGetKeyState(wx.WXK_CONTROL) then
					return
				end
				event:Skip()
			end)
...
function repl:inputEditable(line)
	return self:caretOnPromptLine(false, line)
		and not (self.console:LineFromPosition(self.console:GetSelectionStart()) < self:getPromptLine())
end
