...
function styled_text_ctrl.create(...)
	...
	for _, m in ipairs(useraw and { "AppendTextRaw", "AddTextRaw" } or {}) do
		local orig = editor[m]
		editor[m] = function(self, text, length)
			return orig(self, text, length or #text)
		end
	end
	...
	for _, m in ipairs(rawMethods) do
		...
		editor[m] = function(...)
			return editor[m:gsub("Dyn", useraw and "Raw" or "")](...) or def
		end
	end

	function editor:CopyDyn()
		invalidUTF8 = nil
		if not self.useraw then
			return self:Copy()
		end
		-- check if selected fragment is a valid UTF-8 sequence
		local text = self:GetSelectedTextRaw()
		if text == "" or wx.wxString.FromUTF8(text) ~= "" then
			return self:Copy()
		end
	...
	function editor:PasteDyn()
		...
		if util.os_name() == "Windows" then
			text = text and text:gsub("%z+$", "")
		end
		if not ok or wx.wxString.FromUTF8(text) ~= "" or not invalidUTF8 or invalidLength ~= tdo:GetDataSize() then
			return self:Paste()
		end
	...
	function editor:CanFold()
		for m = 0, util.MAXMARGIN do
			if self:GetMarginWidth(m) > 0 and self:GetMarginMask(m) == wxstc.wxSTC_MASK_FOLDERS then
				return true
			end
		end
		return false
	...
	function editor:FoldSome(line)
		...
		local header = line
			and bit.band(self:GetFoldLevel(line), wxstc.wxSTC_FOLDLEVELHEADERFLAG)
		...
		if not foldall and not hidebase then
			self:ShowLines(from, to)
		end
	...
	function editor:GetAllMarginWidth()
		local width = 0
		for m = 0, util.MAXMARGIN do
			width = width + self:GetMarginWidth(m)
		end
		return width
	end
	...
	function editor:ShowPosEnforcePolicy(pos)
		...
		if self:GetWrapMode() ~= wxstc.wxSTC_WRAP_NONE then
			return
		end
	...
	function editor:GetLineWrapped(pos, direction)
		...
		local line = self:LineFromPosition(pos)
		if
			self:WrapCount(line) < 2
			or direction < 0 and line == 0
			or direction > 0 and line == self:GetLineCount() - 1
		then
			return false
		end
	...
	function editor:GetModifiedTime()
		return self.updated
	end
	...
	for _, method in ipairs({ "AutoCompShow", "UserListShow" }) do
		...
		editor[method] = function(editor, ...)
			...
			if tech ~= 0 then
				editor:SetTechnology(wxstc.wxSTC_TECHNOLOGY_DEFAULT)
			end
			orig(editor, ...)
			if tech ~= 0 then
				editor:SetTechnology(tech)
			end
		end
	end
