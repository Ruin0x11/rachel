...
local function make_tostring(kind, tbl)
	return function(self)
		...
		if self.__class then
			return string.format("instance of '%s' (%s)", rawget(self.__class, "__name") or "?", addr)
		end
		return tbl[self] and string.format("%s '%s' (%s)", kind, rawget(self, "__name") or "?", addr)
...
function class.interface(name, reqs, parents)
	...
	i.delegate = function(i, field, params)
		if params == nil or _classes[params] then
			error("Invalid delegate parameter for " .. i.__name .. "." .. field .. ": " .. tostring(params))
		end
		if _interfaces[params] or type(params) == "string" then
			params = { params }
		end
...
local function delegate(c, field, params)
	...
	if params == nil or _classes[params] then
		error("Invalid delegate parameter for " .. c.__name .. "." .. field .. ": " .. tostring(params))
	end
	if _interfaces[params] or type(params) == "string" then
		params = { params }
	end
		...
		if field_name then
			...
			if field then
				if type(field[k]) == "function" then
					...
					__memoized[k] = __memoized[k] or function(self, ...)
						return f(field, ...)
					end
					return __memoized[k]
				else
